...
export default function InfiniteHero({ waitlistCount = 0 }: InfiniteHeroProps) {
		...
		>
			...
			<div className="relative z-10 flex h-svh w-full items-center justify-center px-6">
				<div className="text-center">
					...
					<div
						ref={badgeRef}
						className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/20 bg-white/5 text-sm text-white/50 mb-8 backdrop-blur-sm"
					>
					...
					<h1
						ref={h1Ref}
						className="mx-auto max-w-2xl lg:max-w-4xl text-[clamp(3rem,7vw,5rem)] font-extralight leading-[0.95] tracking-tight"
					>
						X<span className="text-blue-400">Vault</span>
					</h1>

					<p
						ref={pRef}
						className="mx-auto mt-5 max-w-lg md:text-balance text-base/7 md:text-lg/8 font-light tracking-tight text-white/60"
					>
						The only X growth platform with a{" "}
						<span className="text-white/90 font-normal">BYOK model</span>.
					...
					>
						<div className="flex flex-row items-center justify-center gap-4">
							<Link
								href="/waitlist"
								className="relative overflow-hidden border border-white/30 bg-gradient-to-r from-white/20 to-white/10 px-5 py-2.5 text-sm rounded-lg font-medium tracking-wide text-white backdrop-blur-sm transition-[border-color,background-color,box-shadow] duration-500 hover:border-white/50 hover:bg-white/20 hover:shadow-lg hover:shadow-white/10"
							>
								Join the Waitlist
							</Link>
							...
							<a
								href="#features"
								className="px-4 py-2.5 text-sm font-medium tracking-wide text-white/60 transition-[filter,color] duration-500 hover:drop-shadow-[0_0_6px_rgba(255,255,255,0.6)] hover:text-white"
							>
								How it works
							</a>
						</div>
						...
						<div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1 mt-1">
							{[
								...
								<span key={perk} className="flex items-center gap-1.5 text-sm text-white/55">
									<svg width="10" height="10" viewBox="0 0 10 10" fill="none" className="shrink-0 text-blue-400" aria-hidden>
						...
						{waitlistCount > 0 && (
							<p className="text-sm text-white/35">
								<span className="text-white/55">{waitlistCount.toLocaleString()} developers</span> already on the list
							</p>
						)}
					</div>
